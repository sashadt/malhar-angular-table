...
.directive('mlhrTableRows', function($filter) {
  ...
  function calculateVisibleRows(scope) {
    ...
    visible_rows = tableRowFilter(scope.rows, scope._columns, scope.searchTerms, scope.filterState);
    
    // | tableRowSorter:columns:sortOrder:sortDirection 
    visible_rows = tableRowSorter(visible_rows, scope._columns, scope.sortOrder, scope.sortDirection);
